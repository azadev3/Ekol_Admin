      ...
      <div className="link-area">
        {props.to && !props.isDropdown ? (
          <NavLink
            ...
            {props.isDropdown ? <FaAngleDown className="caret" style={{ transform: open ? 'rotate(180deg)' : '' }} /> : ''}
          </NavLink>
        ) : (
          <div
            className={`dropdown-sidebar-item ${props.isDropdown && isMatchedLocationForActiveClass ? 'active-dropdown' : ''
              }`}
            onClick={() => {
              if (props.isDropdown && toggleSidebar) {
                handleOpen();
