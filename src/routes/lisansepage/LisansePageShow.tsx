...
const LisansePageShow: React.FC = () => {
    ...
    const toggleStatus = async (id: string | number) => {
        ...
        try {
            await axios.put(`${URL}/lisansepage/status/${id}`, { statusActive: newStatus });
        } catch (error) {
    ...
    const handleDelete = async (id: any) => {
        try {
            const deleteitem = await axios.delete(`${URL}/lisansepage/${id}`, Option());
            if (deleteitem.data) {
                fetchData();
            } else {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${URL}/lisansepage`, Option());
    ...
    return (
        <div className="show-component">
            <ImageResizeInformation routeTitle='Lisenziyalar' sizeTitle='392 x 574' />
