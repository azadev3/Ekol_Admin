...
const ToolsInnerShow: React.FC = () => {
 ...
 const toggleStatus = async (id: string | number) => {
  ...
  try {
   await axios.put(`${URL}/toolsinner/status/${id}`, { statusActive: newStatus });
  } catch (error) {
 ...
 const handleDelete = async (id: any) => {
  try {
   const deleteitem = await axios.delete(`${URL}/toolsinner/${id}`, Option());
   if (deleteitem.data) {
    fetchData();
   } else {
 ...
 const fetchData = async () => {
  try {
   const response = await axios.get(`${URL}/toolsinner`, Option());
