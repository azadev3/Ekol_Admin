...
const WhoareweShow: React.FC = () => {
    ...
    const toggleStatus = async (id: string | number) => {
        ...
        try {
            await axios.put(`${URL}/whoarewe/status/${id}`, { statusActive: newStatus });
        } catch (error) {
        ...
        { field: 'description_ru', headerName: 'Description RU', width: 200 },
        {
            ...
            renderCell: (params) => (
                <div className="buttons-grid">
                    <button
                        className="edit"
                        onClick={() => navigate(`/whoarewe/${params.row.id}`)} // Navigating to the edit page with the row's id
                    >
                        Düzəliş
                    </button>
                    <button
    ...
    const handleDelete = async (id: any) => {
        try {
            const deleteitem = await axios.delete(`${URL}/whoarewe/${id}`, Option());
            if (deleteitem.data) {
                fetchData();
            } else {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${URL}/whoarewe`, Option());
    ...
    return (
        <div className="show-component">
            <ImageResizeInformation routeTitle='Biz kimik səhifəsi' sizeTitle='1240 x 500' />
            <Title description="Əlavə et, dəyişdir, sil." title="Biz Kimik?" to="/whoarewe/create" />
