...
const WhyEcolShow: React.FC = () => {
    ...
    { field: "description_ru", headerName: "Description RU", width: 200 },
    {
      ...
      renderCell: (params) => (
        <div className="buttons-grid">
          <button
            className="edit"
            onClick={() => navigate(`/whyecol/${params.row.id}`)} // Navigating to the edit page with the row's id
          >
            Düzəliş
          </button>
          <button
  ...
  const handleDelete = async (id: any) => {
    try {
      const deleteitem = await axios.delete(`${URL}/whyecol/${id}`, Option());
      if (deleteitem.data) {
        fetchData();
      } else {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${URL}/whyecol`, Option());
  ...
  return (
    <div className="show-component">
      <Title
        ...
        title="Niyə Ekol? (Kariyera imkanları səhifəsi)"
        to="/whyecol/create"
      />
