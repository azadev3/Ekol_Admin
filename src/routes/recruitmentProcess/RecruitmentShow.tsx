...
const RecruitmentShow: React.FC = () => {
    ...
    { field: "order", headerName: "Order", width: 200 },
    {
      ...
      renderCell: (params) => (
        <div className="buttons-grid">
          <button
            className="edit"
            onClick={() => navigate(`/recruitmentprocess/${params.row.id}`)} // Navigating to the edit page with the row's id
          >
            Düzəliş
          </button>
          <button
  ...
  const handleDelete = async (id: any) => {
    try {
      const deleteitem = await axios.delete(`${URL}/recruitmentprocess/${id}`, Option());
      if (deleteitem.data) {
        fetchData();
      } else {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${URL}/recruitmentprocess`, Option());
  ...
  return (
    <div className="show-component">
      <Title description="Əlavə et, dəyişdir, sil." title="Karyera İmkanları - İşə qəbul prosesi" to="/recruitmentprocess/create" />
