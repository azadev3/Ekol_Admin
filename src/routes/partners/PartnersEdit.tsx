...
const PartnersEdit: React.FC = () => {
  ...
  useEffect(() => {
    if (editid) {
      const fetchData = async () => {
        try {
          const response = await axios.get(`${URL}/partners/${editid}`);
          const data = response.data;
          setTitleAz(data.title.az || "");
          setTitleEn(data.title.en || "");
          setTitleRu(data.title.ru || "");
          setImagePreview(`https://ekol-server.onrender.com${data.logo}` || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`${URL}/partners/${editid}`, formData, {
        headers: {
  ...
  return (
    <div className="component-edit">
      <Title description="Dəyişiklik et" title="Partnyorlar" to="" />
