...
const NaturalForm: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${URL}/purchaseNaturalFormFront`, {
        headers: {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await axios.delete(`${URL}/deleteItemNat/${id}`);
      if (response.data) {
          ...
          <article className="form-item-mini" key={data?._id}>
            <div className="left">
              ...
              <div className="name-area">
                ...
                <span className="company">VÖEN: {data?.voen}</span>
              </div>
            </div>
            <div className="right">
