...
const OurworksinnerShow: React.FC = () => {
 ...
 const toggleStatus = async (id: string | number) => {
  ...
  try {
   await axios.put(`${URL}/ourworksinner/status/${id}`, { statusActive: newStatus });
  } catch (error) {
  ...
  { field: 'description_ru', headerName: 'Description RU', width: 200 },
  {
   ...
   renderCell: (params) => (
    <div className="buttons-grid">
     <button
      className="edit"
      onClick={() => navigate(`/ourworksinner/${params.row.id}`)} // Navigating to the edit page with the row's id
     >
      Düzəliş
     </button>
     <button
 ...
 return (
  <div className="show-component">
   <Title description="Əlavə et, dəyişdir, sil." title="Gördüyümüz işlər (daxili)" to="/ourworksinner/create" />
...
export default OurworksinnerShow;
