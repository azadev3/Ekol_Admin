...
const NewBlogImageEdit: React.FC = () => {
    ...
    useEffect(() => {
        if (editid) {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`${URL}/newblogimage/${editid}`, Option());
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.put(`${URL}/newblogimage/${editid}`, formData, Option());
    ...
    return (
        <div className="component-edit">
            <Title description="Dəyişiklik et" title="Bloqlar üçün şəkillər" to="" />
            ...
            <form noValidate autoComplete="off" onSubmit={handleSubmit} style={{ marginTop: "16px" }}>
                ...
                {existingImages.length > 0 && (
                    <Box mt={2}>
                        <Typography variant="subtitle1">Movcud Şəkillər:</Typography>
                        <Box
                            mt={2}
                            sx={{
                                display: "flex",
...
export default NewBlogImageEdit;
