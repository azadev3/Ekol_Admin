...
const ServicesShow: React.FC = () => {
    ...
    { field: "description_ru", headerName: "Description RU", width: 200 },
    {
      ...
      renderCell: (params) => (
        <div className="buttons-grid">
          <button
            className="edit"
            onClick={() => navigate(`/services/${params.row.id}`)} // Navigating to the edit page with the row's id
          >
            Düzəliş
          </button>
          <button
  ...
  const handleDelete = async (id: any) => {
    try {
      const deleteitem = await axios.delete(`${URL}/services/${id}`);
      if (deleteitem.data) {
        fetchData();
      } else {
  ...
  return (
    <div className="show-component">
      <Title description="Əlavə et, dəyişdir, sil." title="Xidmətlər" to="/services/create" />
