...
const StructureCategoriesShow: React.FC = () => {
    ...
    { field: "title_ru", headerName: "Başlıq RU", width: 150 },
    {
      ...
      renderCell: (params) => (
        <div className="buttons-grid">
          <button
            className="edit"
            onClick={() => navigate(`/departmentscategories/${params.row.id}`)} // Navigating to the edit page with the row's id
          >
            Düzəliş
          </button>
          <button
  ...
  const handleDelete = async (id: any) => {
    try {
      const deleteitem = await axios.delete(`${URL}/departmentscategories/${id}`);
      if (deleteitem.data) {
        fetchData();
      } else {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${URL}/departmentscategories`);
...
export default StructureCategoriesShow;
