    ...
    if (editid) {
      axios.get(`${URL}/imagespage/${editid}`)
        .then(response => {
          ...
          setImagePreviewCategoryImage(`https://ekol-server.onrender.com${data.categoryImg }`|| "");
          setImagesPreviews(data.images.map((img: any) => `https://ekol-server.onrender.com${img.image}`));
        })
        .catch(error => {
