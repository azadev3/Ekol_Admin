...
const CertificatesShow: React.FC = () => {
    ...
    const toggleStatus = async (id: string | number) => {
        ...
        try {
            await axios.put(`${URL}/certificates/status/${id}`, { statusActive: newStatus });
        } catch (error) {
        ...
        { field: 'description_ru', headerName: 'Description RU', width: 200 },
        {
            ...
            renderCell: (params) => (
                <div className="buttons-grid">
                    <button
                        className="edit"
                        onClick={() => navigate(`/certificates/${params.row.id}`)} // Navigating to the edit page with the row's id
                    >
                        Düzəliş
                    </button>
                    <button
    ...
    const handleDelete = async (id: any) => {
        try {
            const deleteitem = await axios.delete(`${URL}/certificates/${id}`, Option());
            if (deleteitem.data) {
                fetchData();
            } else {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${URL}/certificates`, Option());
    ...
    return (
        <div className="show-component">
            <ImageResizeInformation routeTitle='Sertifikatlar' sizeTitle='392 x 511 vəya 400 x 322' />
            <Title description="Əlavə et, dəyişdir, sil." title="Sertifikatlar" to="/certificates/create" />
