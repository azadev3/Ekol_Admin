...
router.post("/lisansepage", upload.none(), async (req, res) => {
  try {
    ...
    const createData = new LisansePage({
      title: {
...
router.get("/lisansepage/:editid", async (req, res) => {
  try {
    const { editid } = req.params;

    const datasForId = await LisansePage.findById(editid).lean().exec();
...
router.put("/lisansepage/:editid", upload.none(), async (req, res) => {
  try {
    ...
    const uptadedLisanses = await LisansePage.findByIdAndUpdate(
      editid,
      {
        $set: {
          title: {
    ...
    return res.status(200).json(uptadedLisanses);
  } catch (error) {
...
// for front
router.get("/lisansepagefront", async (req, res) => {
  try {
